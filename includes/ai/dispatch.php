 ...
 * in Fettle needs. Error handling/labelling mirrors that plugin's
 ...
 * @return WP_Error
 */
function fettle_ai_error_from_response( $provider, $http_code, $response_body, $model = '' ) {
	$label      = fettle_ai_provider_label( $provider );
	...
	$generic_messages = array(
		...
		400 => sprintf( __( 'Bad request to %s. The request may be malformed or the image too large.', 'fettle' ), $label ),
		/* translators: %s: AI provider name (e.g. Gemini, Claude) */
		401 => sprintf( __( 'Invalid %s API key. Check it in Fettle settings.', 'fettle' ), $label ),
		/* translators: %s: AI provider name (e.g. Gemini, Claude) */
		403 => sprintf( __( 'Access denied by %s. Check your account permissions/billing.', 'fettle' ), $label ),
		/* translators: %s: AI provider name (e.g. Gemini, Claude) */
		404 => sprintf( __( 'Model not found on %s. It may have been renamed or removed.', 'fettle' ), $label ),
		/* translators: %s: AI provider name (e.g. Gemini, Claude) */
		429 => sprintf( __( '%s rate limit exceeded. Wait a moment and try again.', 'fettle' ), $label ),
		/* translators: %s: AI provider name (e.g. Gemini, Claude) */
		500 => sprintf( __( '%s internal server error. Try again in a few moments.', 'fettle' ), $label ),
		/* translators: %s: AI provider name (e.g. Gemini, Claude) */
		503 => sprintf( __( '%s is temporarily unavailable (overloaded or maintenance).', 'fettle' ), $label ),
	);

	$message = $generic_messages[ $http_code ]
		?? sprintf(
			/* translators: 1: AI provider name, 2: HTTP status code */
			__( '%1$s returned an unexpected error (HTTP %2$d).', 'fettle' ),
			$label,
			$http_code
		);

	return new WP_Error( 'fettle_ai_http_' . $http_code, $message . $api_details );
}
...
function fettle_ai_call_vision( $provider, $model, $prompt, $image, $api_key, $options = array() ) {
	...
	switch ( $provider ) {
		case 'anthropic':
			return fettle_ai_call_vision_anthropic( $model, $prompt, $image, $api_key, $options );
		case 'openai':
			return fettle_ai_call_vision_openai_compatible( 'https://api.openai.com/v1/chat/completions', 'openai', $model, $prompt, $image, $api_key, $options );
		case 'xai':
			return fettle_ai_call_vision_openai_compatible( 'https://api.x.ai/v1/chat/completions', 'xai', $model, $prompt, $image, $api_key, $options );
		case 'openrouter':
			return fettle_ai_call_vision_openai_compatible( 'https://openrouter.ai/api/v1/chat/completions', 'openrouter', $model, $prompt, $image, $api_key, $options );
		case 'gemini':
		default:
			return fettle_ai_call_vision_gemini( $model, $prompt, $image, $api_key, $options );
	}
}
