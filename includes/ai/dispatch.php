 ...
 * @return WP_Error
 */
function usher_ai_error_from_response( $provider, $http_code, $response_body, $model = '' ) {
	$label      = usher_ai_provider_label( $provider );
	...
	$generic_messages = array(
		...
		400 => sprintf( __( 'Bad request to %s. The request may be malformed or the image too large.', 'usher' ), $label ),
		/* translators: %s: AI provider name (e.g. Gemini, Claude) */
		401 => sprintf( __( 'Invalid %s API key. Check it in Usher settings.', 'usher' ), $label ),
		/* translators: %s: AI provider name (e.g. Gemini, Claude) */
		403 => sprintf( __( 'Access denied by %s. Check your account permissions/billing.', 'usher' ), $label ),
		/* translators: %s: AI provider name (e.g. Gemini, Claude) */
		404 => sprintf( __( 'Model not found on %s. It may have been renamed or removed.', 'usher' ), $label ),
		/* translators: %s: AI provider name (e.g. Gemini, Claude) */
		429 => sprintf( __( '%s rate limit exceeded. Wait a moment and try again.', 'usher' ), $label ),
		/* translators: %s: AI provider name (e.g. Gemini, Claude) */
		500 => sprintf( __( '%s internal server error. Try again in a few moments.', 'usher' ), $label ),
		/* translators: %s: AI provider name (e.g. Gemini, Claude) */
		503 => sprintf( __( '%s is temporarily unavailable (overloaded or maintenance).', 'usher' ), $label ),
	);

	$message = $generic_messages[ $http_code ]
		?? sprintf(
			/* translators: 1: AI provider name, 2: HTTP status code */
			__( '%1$s returned an unexpected error (HTTP %2$d).', 'usher' ),
			$label,
			$http_code
		);

	return new WP_Error( 'usher_ai_http_' . $http_code, $message . $api_details );
}
 ...
 */
function usher_ai_call_vision( $provider, $model, $prompt, $image, $api_key, $options = array() ) {
	if ( '' === trim( (string) $api_key ) ) {
		return new WP_Error( 'usher_ai_no_key', __( 'No API key configured for this provider.', 'usher' ) );
	}

	switch ( $provider ) {
		case 'anthropic':
			return usher_ai_call_vision_anthropic( $model, $prompt, $image, $api_key, $options );
		case 'openai':
			return usher_ai_call_vision_openai_compatible( 'https://api.openai.com/v1/chat/completions', 'openai', $model, $prompt, $image, $api_key, $options );
		case 'xai':
			return usher_ai_call_vision_openai_compatible( 'https://api.x.ai/v1/chat/completions', 'xai', $model, $prompt, $image, $api_key, $options );
		case 'openrouter':
			return usher_ai_call_vision_openai_compatible( 'https://openrouter.ai/api/v1/chat/completions', 'openrouter', $model, $prompt, $image, $api_key, $options );
		case 'gemini':
		default:
			return usher_ai_call_vision_gemini( $model, $prompt, $image, $api_key, $options );
	}
}
