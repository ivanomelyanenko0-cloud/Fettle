 ...
 */
function fettle_check_heading_order( $html ) {
	...
	if ( $h1_count > 1 ) {
		$findings[] = array(
			...
			'message'      => sprintf(
				...
				__( 'This content has %d H1 headings; a page should normally have one.', 'fettle' ),
				$h1_count
			),
		...
		if ( '' === trim( $heading['text'] ) ) {
			$findings[] = array(
				...
				'message'      => sprintf(
					...
					__( 'Empty %s heading - screen reader users hear nothing where a heading is announced.', 'fettle' ),
					'H' . $heading['level']
				),
		...
		if ( null !== $previous_level && $heading['level'] > $previous_level + 1 ) {
			$findings[] = array(
				...
				'message'      => sprintf(
					...
					__( 'Heading level jumps from %1$s to %2$s, skipping a level.', 'fettle' ),
					'H' . $previous_level,
					'H' . $heading['level']
				),
				'instance_key' => $heading['instance_key'],
