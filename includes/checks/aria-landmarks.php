...
define( 'FETTLE_LANDMARKS_CACHE_OPTION', 'fettle_landmarks_cache' );

/**
 * @param string $html Rendered page HTML.
 * @return array[] Findings: each { type, severity, message }.
 */
function fettle_parse_landmarks( $html ) {
	...
	if ( 0 === $counts['main'] ) {
		$findings[] = array(
			'type'     => 'aria-landmarks',
			'severity' => 'critical',
			'message'  => __( 'No <main> landmark found on the front page - screen reader users have no quick way to skip to the main content.', 'fettle' ),
		);
		...
		$findings[] = array(
			...
			'message'  => sprintf(
				...
				__( '%d <main> landmarks found on the front page - there should be exactly one.', 'fettle' ),
				$counts['main']
			),
		);
	}
	...
	if ( 0 === $counts['navigation'] ) {
		$findings[] = array(
			'type'     => 'aria-landmarks',
			'severity' => 'warning',
			'message'  => __( 'No navigation landmark (<nav> or role="navigation") found on the front page.', 'fettle' ),
		);
	}
	...
	if ( 0 === $counts['contentinfo'] ) {
		$findings[] = array(
			'type'     => 'aria-landmarks',
			'severity' => 'warning',
			'message'  => __( 'No footer landmark (<footer> or role="contentinfo") found on the front page.', 'fettle' ),
		);
	}
	...
	if ( 0 === $counts['banner'] ) {
		$findings[] = array(
			'type'     => 'aria-landmarks',
			'severity' => 'warning',
			'message'  => __( 'No banner landmark (<header> or role="banner") found on the front page.', 'fettle' ),
		);
	}
 ...
 * }
 */
function fettle_run_landmark_check() {
	$result = array(
	...
	if ( 200 !== $http_code ) {
		...
		$result['error'] = sprintf( __( 'Unexpected response fetching the front page (HTTP %d).', 'fettle' ), $http_code );
		return $result;
	}

	$result['findings'] = fettle_parse_landmarks( wp_remote_retrieve_body( $response ) );

	update_option( FETTLE_LANDMARKS_CACHE_OPTION, $result, false );
 ...
 */
function fettle_get_cached_landmark_result() {
	$cached = get_option( FETTLE_LANDMARKS_CACHE_OPTION, null );
 ...
 */
add_action( 'switch_theme', 'fettle_clear_landmark_cache' );
function fettle_clear_landmark_cache() {
	delete_option( FETTLE_LANDMARKS_CACHE_OPTION );
}
